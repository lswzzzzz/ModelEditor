	...
	FbxAMatrix& pGlobalPosition, FbxPose* pPose, ShadingMode pShadingMode)
{
	...
	//所有的数据包括Vertex Color Normal Tangent UV都在这里面
	...
	//读取Color
	//lMesh->GetElementVertexColor(NULL);
	//读取UV
	//lMesh->GetElementUV(uvLayer);
	//读取Normal
	//lMesh->GetElementNormal(0);
	//读取Tangent
	//lMesh->GetElementTangent(0);
	
	//三角形数量
	addConsoleInfo("TriangleCount:%d", lMesh->GetPolygonCount());
	//顶点数量
	addConsoleInfo("VertexCount:%d", lMesh->GetControlPointsCount());
	//GetPolygonCount()*3 == GetPolygonVertexCount()
	//索引数组指针
	//GetPolygonVertices;
	addConsoleInfo("IndexCount:%d", lMesh->GetPolygonVertexCount());
	addConsoleInfo("Layer:%d", lMesh->GetLayerCount());
	addConsoleInfo("DeformerCount:%d", lMesh->GetDeformerCount(FbxDeformer::eSkin));
	addConsoleInfo("MaterialCount:%d", pNode->GetMaterialCount());
	addConsoleInfo("\n\n");
	...
	ShadingMode pShadingMode)
{
	...
	if (pNode->GetNodeAttribute())
	{
		...
		//渲染当前节点
		DrawNode_(pNode, pTime, pAnimLayer, pParentGlobalPosition, lGlobalOffPosition, pPose, pShadingMode);
	}
